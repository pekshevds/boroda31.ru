...
class Category(models.Model):
    ...
    menu = models.ForeignKey('MenuItem', verbose_name="Меню", on_delete=models.PROTECT, blank=True, null=True)    
    ...
    def get_goods(self):
        try:
            goods = Good.objects.filter(category=self, is_show=True)
        except:
            return None
...
class Good(models.Model):
    ...
    weight = models.DecimalField(verbose_name='Вес, гр', default=0, max_digits=15, decimal_places=3)
    ...
    def save(self, *args, **kwargs):
        ...
        try:
            self.slug = slugify(unidecode(self.site_name + ' ' + str(self.id)))  
        except:
            pass
...
class MenuItem(models.Model):
    ...
    def get_childrens_count(self):        
        return len(MenuItem.objects.filter(parent=self))


    def get_categoryes(self):
        try:
            categoryes = Category.objects.filter(menu=self, is_show=True)
        except:
            return None
